...
# if they want the program to print "something". However, this
#   program recognized any input as valid: if you answered anything
#    but y, it treated it as an n response, and quit without
#    printing anything.
#
...
# (uppercase) responses; case sensitive input is generally a poor
#   user interface choice. Whenever possible, accept both
...
# variable we want to use. Here we start by setting choice to nil; this guarantees
# that choice will be available both inside the loop and after the loop has finished running.
#
# Inside the loop, we display our prompt, and then use
# #gets to read the user's input. We also use #chomp in this case
#  to get rid of the newline, and #downcase to convert the input to
#  lowercase.
#
...
# valid entries (y and n). We use the %w() shortcut syntax to represent the Array
# since it is easier to read %w(y n) than ['y', 'n'].
#
