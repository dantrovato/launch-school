...
removes the most recently pushed value from the stack), perform the operation
using the popped value and the register value, and then store the result back
in the register.

Consider a MULT operation in a stack-and-register language. It multiplies the stack
value with the register value, removes the value from the stack, and then stores
the result back in the register. Thus, if we start with a stack of 3 6 4 (where 4
  is the topmost item in the stack), and a register value of 7, then the MULT operation
  will transform things to 3 6 on the stack (the 4 is removed), and the result of
  the multiplication, 28, is left in the register. If we do another MULT at this
    point, then the stack is transformed to 3, and the register is left with the
    value 168.
...
argument. Your program may assume that all programs are correct programs; that is,
they won't do anything like try to pop a non-existent value from the stack, and
...
Discussion
This exercise takes a lot of time to explain, but is not that difficult to implement.
Once you understand what is expected, it's simply a matter of setting up a processing
loop, and doing the necessary processing.

We start by creating our stack and register, and then just walk through each "token"
in the program, performing the appropriate action during each iteration. A token
is one of the words or values in the language.

We use a case statement for the body of our loop. The one thing that might trip
  you up is remembering to put integer tokens into the register; an easy step to
  do, an easy step to forget.
