...
Discussion
Solution 1 answers the main question in this exercise. This is almost too simple;
our method simply calls the fibonacci method from our last exercises, converts
the result to a string, and then returns the last digit.
...
To compute the last digit of the nth Fibonacci number, you only need the last digit
of the nth - 1 and nth -2 numbers. As a result, you only ever need the last digit
of any intermediate result, which eliminates all of the computing effort needed
to compute the massive numbers involved. Our second solution does exactly this:
it only computes and uses the last digit in each intermediate result, and computes
the last digit of the 123,456,789th Fibonacci number in less than a minute.
