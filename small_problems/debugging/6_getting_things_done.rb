=begin
101-109 - Small ProblemsDebuggingGetting Things Done
...
Every recursion needs a base case, i.e. a condition under which it will stop. Our
  method doesn't have one, so it just repeatedly calls itself with ever decreasing
  values for n:
...
Ruby puts each call on a call stack. Since we never stop, this stack keeps growing
and growing, until it exceeds its limit. This causes the SystemStackError - a real
stack overflow!
