...
Discussion
This method is expected to return a hash of character counts, so we'll create a
counts hash to store that data.

Next, we use Array#count to find the number of occurrences of lowercase, uppercase,
and all other characters.

With that we have the data needed to satisfy this problem's requirements: a hash
that has the count of characters that are lowercase, uppercase, or neither.

We use regular expressions to count certain types of characters. We'll explain
things a bit here for those who aren't familiar with regex:
