=begin
101-109 - Small ProblemsMedium 2 Bubble Sort
...
each pass, each pair of consecutive elements is compared. If the first of the two
elements is greater than the second, then the two elements are swapped. This process
is repeated until a complete pass is made without performing any swaps; at that
...
well as a minor optimization technique, see the Bubble Sort wikipedia page. (in
bookmarks)
...
=begin
MY SLUSH
def bubble_sort!(arr) # => [2, 6, 1, 4, 7]
  loop do
    l = 0 # left index 1
    r = 1 # right index 0
    counter = 0
    loop do
      # => swaps first for second if condition is met
      if arr[l] > arr[r]
    ...
    break if r == arr.size && counter.zero?
  end
end
...
Discussion
Our outer loop handles the task of repeating iterations until the Array is completely
sorted. It terminates the first time we iterate through all comparisons without
making any swaps, which we keep track of through the swapped variable.

The inner loop takes care of looking at every pair of consecutive elements and
swapping them if the first element of a pair is greater than the second. We use
the usual ruby idiom for swapping two values, e.g.,
...
skipped this optimization, trying modifying your solution so it uses that optimization.
=end
